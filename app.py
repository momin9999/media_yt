...
def get_formats():
    ...
    try:
        # 【修正点1】 --no-check-certificate を追加
        command = ['yt-dlp', '--no-check-certificate', '--dump-json', '--no-playlist', final_url]
        ...
        for f in formats:
            ...
            selectable_formats.append({'id': f['format_id'], 'note': note, 'ext': ext})

        return jsonify({'status': 'success', 'formats': selectable_formats})
    except subprocess.CalledProcessError as e:
        # 【修正点2】 エラーログを詳細化
        error_message = f"yt-dlpの実行に失敗しました。終了コード: {e.returncode}\n---STDOUT---\n{e.stdout}\n---STDERR---\n{e.stderr}"
...
def download_video():
    ...
    with TemporaryDirectory() as temp_dir:
        try:
            ...
            # 【修正点1】 --no-check-certificate を追加
            command = ['yt-dlp', '--no-check-certificate', '--no-playlist', '-f', format_id, '-o', output_template, final_url]
...
@app.route('/')
def index():
    return "Download API is running. (v5: error handling improved)"
